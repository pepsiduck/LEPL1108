...
def main():
    ...
    range_values = [np.arange(0, pad_size_Ms, 1) * (3e8) / (2 * B * k) for i in range(0,4)]
    ...
    positions = np.array([position_calc([2*r0[frame],2*r1[frame],2*r2[frame],2*r3[frame]], e_coord, a_coord) for frame in range(0, n_frame)])
